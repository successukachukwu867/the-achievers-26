...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  body {
    ...
    min-height: 100vh; font-family: 'Montserrat', sans-serif; padding: 16px;
  }
  .card {
    width: 100%; max-width: 520px;
    background: linear-gradient(160deg, #1e6b1e 0%, #0e4a0e 35%, #0a3a0a 65%, #1a5c1a 100%);
    border-radius: 18px; overflow: hidden; position: relative;
    box-shadow: 0 20px 60px rgba(0,0,0,0.7);
  }
  .bg-decor { position: absolute; inset: 0; z-index: 0; pointer-events: none; overflow: hidden; }
  .bg-ruler { position: absolute; top: 0; left: 0; right: 0; height: 6px;
    background: repeating-linear-gradient(90deg, transparent 0px, transparent 8px, rgba(245,200,66,0.28) 8px, rgba(245,200,66,0.28) 10px); }
  .bg-ruler-bottom { position: absolute; bottom: 0; left: 0; right: 0; height: 6px;
    background: repeating-linear-gradient(90deg, transparent 0px, transparent 8px, rgba(245,200,66,0.22) 8px, rgba(245,200,66,0.22) 10px); }
  .bg-lines { position: absolute; inset: 0;
    background-image: repeating-linear-gradient(45deg, transparent, transparent 40px, rgba(255,255,255,0.013) 40px, rgba(255,255,255,0.013) 41px); }
  .bg-sparkle { position: absolute; border-radius: 50%; background: radial-gradient(circle, #f9d84a 0%, transparent 70%); }
  .gold-blob-tr { position: absolute; top: -15px; right: -15px; width: 150px; height: 150px;
    background: radial-gradient(ellipse at 40% 40%, #f9d84a 0%, #d4a017 45%, #8a6000 85%, transparent 100%);
    border-radius: 50% 20% 60% 30%; opacity: 0.95; z-index: 1; }
  .gold-blob-bl { position: absolute; bottom: -20px; left: -20px; width: 170px; height: 170px;
    background: radial-gradient(ellipse at 40% 40%, #f9d84a 0%, #d4a017 45%, #8a6000 85%, transparent 100%);
    border-radius: 40% 60% 30% 70%; opacity: 0.9; z-index: 1; }
  ...
  .logo-circle { width: 40px; height: 40px; border-radius: 50%; background: #fff; border: 2px solid #f5c842;
    display: flex; align-items: center; justify-content: center; overflow: hidden; flex-shrink: 0; box-shadow: 0 2px 6px rgba(0,0,0,0.3); }
  ...
  .class-badge { display: inline-block; background: #2a8a2a; color: #000; font-size: 14px; font-weight: 900;
    letter-spacing: 0.08em; padding: 3px 20px; border-radius: 4px; margin-top: 3px; text-transform: uppercase; }
  ...
  .left-col::before { content: ''; position: absolute; inset: -10px; z-index: 0;
    border-left: 2px solid rgba(245,200,66,0.35); border-right: 1px solid rgba(245,200,66,0.15);
    background: radial-gradient(ellipse 80% 60% at 50% 30%, rgba(245,200,66,0.18) 0%, transparent 65%),
      radial-gradient(ellipse 60% 80% at 20% 80%, rgba(245,200,66,0.12) 0%, transparent 60%),
      repeating-linear-gradient(-45deg, transparent 0px, transparent 14px, rgba(255,255,255,0.025) 14px, rgba(255,255,255,0.025) 15px);
    pointer-events: none; }
  .left-col::after { content: ''; position: absolute; inset: 0; z-index: 0;
    background-image: radial-gradient(circle 1px at center, rgba(245,200,66,0.35) 0%, transparent 100%);
    background-size: 18px 18px; pointer-events: none; opacity: 0.5; }
  .left-col > * { position: relative; z-index: 1; }
  .meet-badge { align-self: flex-start; margin-bottom: 6px; line-height: 1; }
  .meet-line { font-family: 'Bebas Neue', sans-serif; font-size: 34px; color: #fff; line-height: 0.92;
    display: block; font-style: italic; text-shadow: 3px 3px 0px rgba(0,0,0,0.4); letter-spacing: 0.02em; }
  .the-finalist-row { display: flex; align-items: center; gap: 3px; line-height: 1; }
  .the-pill { display: inline-flex; align-items: center; justify-content: center; background: #f5c842; color: #111;
    font-size: 10px; font-weight: 900; padding: 2px 5px; border-radius: 3px; text-transform: lowercase; font-family: 'Montserrat', sans-serif; }
  .finalist-line { font-family: 'Bebas Neue', sans-serif; font-size: 34px; color: #fff; line-height: 0.92;
    display: block; font-style: italic; text-shadow: 3px 3px 0px rgba(0,0,0,0.4); letter-spacing: 0.02em; }
  .photo-frame-wrap { width: 100%; position: relative; padding: 4px; }
  .photo-frame { width: 100%; height: 270px; position: relative; background: #fff; padding: 8px 8px 0px 8px;
    transform: rotate(-2.5deg);
    box-shadow: 4px 4px 18px rgba(0,0,0,0.7), -2px -2px 8px rgba(0,0,0,0.35);
    clip-path: polygon(0% 1%, 3% 0%, 6% 1.5%, 10% 0%, 13% 1%, 17% 0%, 20% 1.5%, 24% 0.5%, 27% 0%, 31% 1%, 35% 0%, 39% 1.5%, 43% 0%, 47% 1%, 51% 0%, 55% 1.5%, 59% 0%, 63% 1%, 67% 0%, 71% 1.5%, 75% 0%, 79% 1%, 83% 0%, 87% 1.5%, 91% 0%, 95% 1%, 98% 0.5%, 100% 0%, 100% 0%, 99% 3%, 100% 7%, 99.5% 12%, 100% 17%, 99% 22%, 100% 27%, 99.5% 32%, 100% 37%, 99% 42%, 100% 47%, 99.5% 52%, 100% 57%, 99% 62%, 100% 67%, 99.5% 72%, 100% 77%, 99% 82%, 100% 87%, 99.5% 92%, 100% 97%, 99% 100%, 99% 100%, 95% 99%, 91% 100%, 87% 98.5%, 83% 100%, 79% 99%, 75% 100%, 71% 98.5%, 67% 100%, 63% 99%, 59% 100%, 55% 98.5%, 51% 100%, 47% 99%, 43% 100%, 39% 98.5%, 35% 100%, 31% 99%, 27% 100%, 23% 98.5%, 19% 100%, 15% 99%, 11% 100%, 7% 98.5%, 3% 100%, 0% 100%, 0% 100%, 1% 97%, 0% 92%, 1% 87%, 0% 82%, 1% 77%, 0% 72%, 1% 67%, 0% 62%, 1% 57%, 0% 52%, 1% 47%, 0% 42%, 1% 37%, 0% 32%, 1% 27%, 0% 22%, 1% 17%, 0% 12%, 1% 7%, 0% 2%);
    display: flex; flex-direction: column; }
  .photo-caption { background: #fff; width: 100%; text-align: center; padding: 5px 6px 6px; flex-shrink: 0;
  ...
  .photo-caption-value { font-size: 10px; color: #111; font-weight: 900; font-style: italic; display: inline-block; transform: skewX(-10deg);
  ...
  .photo-placeholder { width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center;
    background: linear-gradient(160deg, #3a8a3a, #1a5a1a); color: rgba(255,255,255,0.5); gap: 8px; }
  .name-badge-wrap { margin-top: 8px; width: 100%; padding: 2.5px;
    background: linear-gradient(90deg, #f5c842, #d4a017, #f5c842);
    clip-path: polygon(6px 0%, 100% 0%, calc(100% - 6px) 100%, 0% 100%); box-shadow: 0 2px 6px rgba(0,0,0,0.4); }
  .name-badge { background: linear-gradient(90deg, #3aaa3a, #1a7a1a); padding: 4px 6px; text-align: center; width: 100%;
    clip-path: polygon(4px 0%, 100% 0%, calc(100% - 4px) 100%, 0% 100%); }
  .full-name { font-size: clamp(7px, 1.8vw, 12px); font-weight: 900; color: #fffde7; text-transform: uppercase;
    letter-spacing: 0.02em; line-height: 1.2; text-shadow: 0 1px 4px rgba(0,0,0,0.5); word-break: break-word; }
  .nickname-badge-wrap { margin-top: 4px; width: 100%; padding: 2.5px;
    background: linear-gradient(90deg, #1a5c1a, #0e3d0e, #1a5c1a);
    clip-path: polygon(6px 0%, 100% 0%, calc(100% - 6px) 100%, 0% 100%); }
  .nickname-badge { background: linear-gradient(90deg, #f5c842, #e8b820); padding: 4px 6px; width: 100%; text-align: center;
    clip-path: polygon(4px 0%, 100% 0%, calc(100% - 4px) 100%, 0% 100%); }
  .nickname-badge span { font-size: clamp(7px, 1.8vw, 12px); font-weight: 900; color: #1a1a00; letter-spacing: 0.12em;
    text-transform: uppercase; word-break: break-word; }
  .right-col { flex: 1; min-width: 0; display: flex; flex-direction: column; padding-top: 2px; justify-content: flex-start; }
  .right-col-inner { display: flex; flex-direction: column; gap: 0; flex: 0 0 auto; }
...
<div class="card">
  <div class="bg-decor">
    ...
    <svg class="bg-star" style="top:8%;left:45%;width:14px;height:14px;" viewBox="0 0 20 20" fill="#f9d84a">
      <path d="M10 0 L11.5 8.5 L20 10 L11.5 11.5 L10 20 L8.5 11.5 L0 10 L8.5 8.5 Z"/>
    </svg>
    <svg class="bg-star" style="top:62%;left:30%;width:10px;height:10px;opacity:0.2;" viewBox="0 0 20 20" fill="#f9d84a">
      <path d="M10 0 L11.5 8.5 L20 10 L11.5 11.5 L10 20 L8.5 11.5 L0 10 L8.5 8.5 Z"/>
    </svg>
  </div>
  ...
  <div class="body">
    <div class="left-col">
      ...
      <div class="name-badge-wrap">
        <div class="name-badge">
          <div class="full-name">${esc(data.fullName)}</div>
        </div>
      </div>

      <div class="nickname-badge-wrap">
        <div class="nickname-badge">
          <span>${esc(data.nickname)}</span>
        </div>
      </div>
    </div>
    ...
    <div class="right-col">
      <div class="right-col-inner">
        ...
        <div class="quote-row">
          <div class="quote-label">Favourite Quote:</div>
          <div class="quote-text">${esc(data.favQuote)}</div>
        </div>
...
async function screenshotCard(html) {
  ...
  const cardHeight = await page.evaluate(() => {
    ...
    return card ? card.getBoundingClientRect().height : 900;
  });
  await page.setViewport({ width: 580, height: Math.ceil(cardHeight) + 40, deviceScaleFactor: 2 });
...
// ── Send email ────────────────────────────────────────────────────────────────
async function sendEmail(fullName, pngBuffer) {
  ...
  await transporter.sendMail({
    ...
    text: `A new finalist card has been submitted by ${fullName}. See attached PNG.`,
    attachments: [{ filename: `${fullName.replace(/\s+/g,'_')}_card.png`, content: pngBuffer, contentType: 'image/png' }],
  });
}
...
app.post('/submit', upload.single('photo'), async (req, res) => {
  try {
    ...
    const pngBuffer = await screenshotCard(cardHTML);

    // Save PNG to cards folder
    const fileName = `${data.fullName.replace(/\s+/g, '_')}_${Date.now()}.png`;
    const filePath = path.join(cardsDir, fileName);
    fs.writeFileSync(filePath, pngBuffer);

    // Auto-delete after 10 minutes
    setTimeout(() => { try { fs.unlinkSync(filePath); } catch (_) {} }, 24 * 60 * 60 * 1000);

    // Send to admin
    await sendEmail(data.fullName, pngBuffer);

    res.json({ ok: true, preview: `/cards/${fileName}`, name: data.fullName });
  } catch (err) {
    console.error(err);
